...
class Car:
    def __init__(self, x, y, color, use_ai=False, road_system=None):
        ...
        self.stationary_threshold = 2 * FPS  # 2 seconds grace period
        self.min_movement_distance = 40.0  # Minimum distance to consider as movement
        self.last_position = (x, y)
        self.movement_check_interval = 30  # Check every 30 frames (0.5 seconds at 60 FPS)
    ...
    def move(self, keys):
        ...
        # Check if car is stationary (after grace period)
        if self.use_ai and self.time_alive > self.stationary_threshold:
            ...
            if self.frames_since_movement_check >= self.movement_check_interval:
                # Check if car has moved enough since last check
                ...
                if distance_moved < self.min_movement_distance:
                    ...
                    # Crash if stationary for more than 1 second (60 frames)
    ...
    def check_stationary(self):
        """Check if the car is stationary based on its movement"""
        distance_moved = math.sqrt((self.x - self.last_position[0]) ** 2 + (self.y - self.last_position[1]) ** 2)
        if distance_moved < self.min_movement_distance:
            self.stationary_timer += self.movement_check_interval
        else:
            self.stationary_timer = 0  # Reset timer if car is moving
...
def evolve_population(cars, population_size=30, road_system=None):
    ...
    for elite in elites:
        ...
        # Reset stationary detection variables
