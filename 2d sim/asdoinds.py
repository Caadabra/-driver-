...
class AStarPathfinder:
    ...
    def a_star(self, start, goal, excluded_edges=None, weight_factor=1.0):
    ...
    def find_multiple_paths(self, start, goal, num_paths=5):
        paths = []
        excluded_edges = set()
        ...
        if path:
            paths.append(path)
            # Exclude some edges from the optimal path
            for j in range(len(path) - 1):
                edge = (min(path[j], path[j+1]), max(path[j], path[j+1]))
                excluded_edges.add(edge)
        
        # Find alternative paths with varying strategies
        for i in range(1, num_paths):
            # Try different weight factors for variation
            weight_factor = 1.0 + (i * 0.2)
            path = self.a_star(start, goal, excluded_edges, weight_factor)
            if path:
                paths.append(path)
                # Exclude some edges from this path too
                for j in range(0, len(path) - 1, 3):  # Exclude every 3rd edge
                    edge = (min(path[j], path[j+1]), max(path[j], path[j+1]))
                    excluded_edges.add(edge)
            else:
                # If no path found, try with fewer excluded edges
...
class MapGUI:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("City-Scale OSM A* Pathfinding with Real-time Path Optimization")
    ...
    def setup_ui(self):
        ...
        self.num_paths_var = tk.StringVar(value="5")
        paths_spin = tk.Spinbox(path_frame, from_=3, to=10, width=5, 
    ...
    def draw_paths(self, paths):
        self.canvas.delete("path")
        colors = ['#0000FF', '#FF8800', '#8800FF', '#00FF88', '#FF0088', 
                 '#88FF00', '#0088FF', '#FF8800', '#8888FF', '#FF88FF']
        ...
        if paths:
            ...
            self.status_var.set(f"Found {len(paths)} alternative paths. Use sliders to optimize in real-time.")
        else:
            self.status_var.set("No paths found between selected points.")
    
    def find_more_paths(self):
        if self.start_pos and self.goal_pos and not self.is_loading:
            self.status_var.set("Finding additional paths...")
            threading.Thread(target=self.find_and_draw_paths, daemon=True).start()
